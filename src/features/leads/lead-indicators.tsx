...
import { Fire, Snowflake, Thermometer, WarningCircle } from "@phosphor-icons/react";
...
export function TempIcon({ temp }: { temp: string | null }) {
  if (temp === "quente")
    return <Fire className="h-3.5 w-3.5 text-destructive" aria-label="Quente" />;
  if (temp === "morno")
...
export function InatividadeBadge({ lead, now }: { lead: Lead; now?: Date }) {
  ...
  return (
    <Badge variant="secondary" className={`${tone} gap-1`} title={`Sem interação há ${dias} dias`}>
      <WarningCircle className="h-3 w-3" /> {dias}d parado
    </Badge>
  );
}
