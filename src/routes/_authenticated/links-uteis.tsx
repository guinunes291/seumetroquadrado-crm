...
const EMPTY_FORM: FormState = {
  titulo: "", descricao: "", url: "", categoria: "", status: "ativo",
};
...
export function LinksUteisPage() {
  ...
  const userIds = useMemo(
    () => Array.from(new Set(acessos.map((a) => a.user_id))),
    [acessos],
  );
  ...
  const { data: nomes = {} } = useQuery({
    ...
    queryFn: async (): Promise<Record<string, string>> => {
      ...
      (data ?? []).forEach((p) => { map[p.id as string] = (p.nome as string) ?? ""; });
      return map;
    },
  });
  ...
  const registrarAcesso = useMutation({
    mutationFn: async (link: LinkUtil) => {
      ...
      await supabase.from("links_uteis_acessos").insert({
        link_id: link.id, user_id: u.user.id,
      });
    },
    onSuccess: () => {
                  ...
                  <TableBody>
                    {acessos.map((row) => (
                      <TableRow key={row.id}>
                        ...
                        <TableCell>
                          {row.link?.categoria
                            ? <Badge variant="outline" className="text-xs">{row.link.categoria}</Badge>
                            : "—"}
                        </TableCell>
...
function LinkFormFields({
  form, setForm, categoriasSugeridas,
}: {
  form: FormState;
