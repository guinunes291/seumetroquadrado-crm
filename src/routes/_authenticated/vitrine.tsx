            ...
            {filtered.length === 0 ? (
              <Card>
                <CardContent className="space-y-2 py-12 text-center text-muted-foreground">
                  ...
                  <Button variant="outline" size="sm" onClick={() => setFilters(emptyVitrineFilters)}>
        ...
        return (
          ...
          >
            ...
            <div className="mt-1 flex flex-wrap items-center gap-1.5 text-xs text-muted-foreground">
              {p.construtora && <span className="font-semibold text-foreground/70">{p.construtora}</span>}
              <span>·</span>
              <span>{[zona ? `Zona ${zona}` : null, p.bairro].filter(Boolean).join(" · ") || "Local a confirmar"}</span>
            </div>
            ...
            return (
              ...
              >
                <td className="px-3 py-2.5">
                  ...
                  {p.construtora && <div className="text-xs text-muted-foreground">{p.construtora}</div>}
                </td>
                <td className="px-3 py-2.5">
                  {zona ? `Zona ${zona}` : "—"}
