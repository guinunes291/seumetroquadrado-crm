...
function getHeatColor(value: number, values: number[]): string {
  if (values.length === 0 || value === 0) return "text-slate-500";
  const sorted = [...values].filter(v => v > 0).sort((a, b) => a - b);
  if (sorted.length === 0) return "text-slate-500";
...
function LiveClock() {
  ...
  return (
    <div className="text-right hidden lg:block">
      <div className="text-xl font-mono font-bold text-cyan-300 tabular-nums leading-tight">{time}</div>
      <div className="text-[10px] text-slate-400 capitalize">{date}</div>
    </div>
  );
}
...
function KPICard({ label, value, numericValue, icon: Icon, variant = "default", subValue, highlight = false, delta, suffix = "" }: {
  label: string;
  value?: string;
  numericValue?: number;
  icon: any;
  ...
  const displayValue = numericValue !== undefined ? `${formatNum(animated)}${suffix}` : (value ?? "—");
  return (
    <div
    ...
    >
      <div className="flex items-start justify-between">
        <div className="flex-1 min-w-0">
          <p className="text-[11px] uppercase tracking-wider font-semibold mb-1 text-slate-200">{label}</p>
          <p className="text-2xl font-bold text-white leading-tight tabular-nums">{displayValue}</p>
          {subValue && <p className="text-[11px] text-slate-300 mt-0.5 font-medium">{subValue}</p>}
          {delta !== undefined && delta !== 0 && (
            <p className={`text-[10px] font-semibold mt-1 tabular-nums flex items-center gap-0.5 ${delta > 0 ? "text-emerald-400" : "text-red-400"}`}>
              ...
              {delta > 0 ? "+" : ""}{formatNum(Math.abs(delta))} vs ant.
            </p>
          )}
        </div>
...
function GaugeChart({ percentage, label, metaDefined = true }: { percentage: number; label: string; metaDefined?: boolean }) {
  ...
  const color = !metaDefined ? "#475569" : clamped >= 100 ? "#10b981" : clamped >= 75 ? "#22c55e" : clamped >= 50 ? "#f59e0b" : "#ef4444";
  const radius = 70;
      ...
      <div className="w-48 h-48 relative">
        <svg viewBox="0 0 180 180" className="w-full h-full transform rotate-[135deg]">
          ...
          <circle cx="90" cy="90" r={radius} fill="none" stroke={color}
            ...
            strokeLinecap="round" style={{ transition: "stroke-dasharray 1.2s ease-out, stroke .5s ease" }} />
        </svg>
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          {metaDefined ? (
            <>
              <span className="text-4xl font-bold text-white tabular-nums">{(animated / 10).toFixed(1) === "NaN" ? "0" : animated.toFixed(1)}%</span>
...
function MetaProgressBanner({ realizado, meta, unidade = "" }: { realizado: number; meta: number; unidade?: string }) {
  ...
  return (
    <div className="px-4 py-3">
      <div className="relative h-9 bg-slate-800/80 rounded-full overflow-visible border border-slate-700/50 mb-6">
        ...
        {milestones.map(m => (
          <div key={m} className="absolute top-0 bottom-0 flex flex-col items-center"
...
function PodiumVisual({ ranking, type = "vendas" }: { ranking: RankRow[]; type?: "vendas" | "pontos" }) {
  if (ranking.length === 0) {
    return <div className="flex items-center justify-center h-48 text-slate-500"><p>Nenhum corretor no ranking</p></div>;
  }
  ...
  const styleFor = (p: number) => {
    if (p === 1) return { size: "w-28 h-28", border: "border-[5px] border-yellow-400", glow: "shadow-[0_0_50px_rgba(250,204,21,0.5)]", nameColor: "text-yellow-300", bg: "from-yellow-500 to-amber-600" };
    if (p === 2) return { size: "w-24 h-24", border: "border-4 border-gray-300", glow: "shadow-[0_0_35px_rgba(209,213,219,0.4)]", nameColor: "text-gray-100", bg: "from-gray-400 to-gray-600" };
    if (p === 3) return { size: "w-22 h-22", border: "border-4 border-amber-500", glow: "shadow-[0_0_30px_rgba(245,158,11,0.4)]", nameColor: "text-amber-300", bg: "from-amber-600 to-orange-700" };
    return { size: "w-16 h-16", border: "border-2 border-blue-400/60", glow: "shadow-[0_0_20px_rgba(96,165,250,0.25)]", nameColor: "text-blue-200", bg: "from-blue-500 to-blue-700" };
  };
  return (
    <div className="relative">
      <div className="flex items-end justify-center gap-4 py-4">
        {podiumOrder.map(({ data, position }) => {
          ...
          const val = type === "vendas" ? `${data.vendas} venda${data.vendas === 1 ? "" : "s"}` : `${data.pontos} pts`;
          const badge = position === 1 ? "👑" : position === 2 ? "🥈" : position === 3 ? "🥉" : null;
          return (
            <div key={data.corretorId} className={`flex flex-col items-center transition-all duration-300 hover:scale-105 ${position === 1 ? "z-10" : ""}`}>
              {badge && <div className="mb-1 text-sm">{badge}</div>}
              <div className={`relative ${position === 1 ? "mb-3" : "mb-2"}`}>
                <div className={`absolute -top-2 -right-2 z-10 w-7 h-7 rounded-full flex items-center justify-center text-sm font-bold bg-gradient-to-br ${s.bg} text-white shadow-lg border-2 border-white/20`}>{position}</div>
                <Avatar className={`${s.size} ${s.border} ${s.glow}`}>
                  <AvatarImage src={data.foto ?? undefined} />
                  <AvatarFallback className={`bg-gradient-to-br ${s.bg} text-white font-bold`}>{getInitials(data.nome)}</AvatarFallback>
                </Avatar>
              </div>
              <p className={`font-bold text-sm ${s.nameColor} text-center max-w-[110px] truncate`}>{data.nome.split(" ")[0]}</p>
...
function RankingLateral({ ranking, type, positionChanges }: {
  ranking: RankRow[];
  type: "vendas" | "pontos";
  ...
  return (
    <div className="space-y-2">
      <div className="grid grid-cols-12 gap-2 text-[10px] text-slate-500 uppercase tracking-wider px-2 pb-2 border-b border-slate-700/50">
      ...
      <div className="space-y-1 max-h-[420px] overflow-y-auto pr-1">
        {ranking.map((item, index) => {
          ...
          return (
            <div key={item.corretorId} className={`grid grid-cols-12 gap-2 items-center py-2 px-2 rounded-lg ${index < 3 ? "bg-slate-800/40" : "hover:bg-slate-800/20"} transition-colors`}>
              <div className="col-span-1 flex items-center gap-1">
                <span className={`text-sm font-bold tabular-nums ${index === 0 ? "text-yellow-300" : index === 1 ? "text-gray-300" : index === 2 ? "text-amber-400" : "text-slate-400"}`}>
                  {index + 1}
                </span>
              ...
              <div className="col-span-7 flex items-center gap-2 min-w-0">
                <Avatar className="w-7 h-7">
                  ...
                  <AvatarFallback className="text-[10px] bg-slate-700 text-white">{getInitials(item.nome)}</AvatarFallback>
                </Avatar>
                <span className="text-white text-xs font-medium truncate">{item.nome}</span>
              </div>
              <div className="col-span-4 text-right text-cyan-300 text-sm font-bold tabular-nums">{formatNum(val)}</div>
            </div>
          );
        })}
      </div>
...
function FunilConversao({ leads, agendamentos, visitas, vendas }: { leads: number; agendamentos: number; visitas: number; vendas: number }) {
  const steps = [
  ...
  const barColor = (r: number) => r >= 60 ? "from-emerald-500 to-emerald-400" : r >= 30 ? "from-amber-500 to-amber-400" : "from-red-500 to-red-400";
        ...
        return (
          <div key={i} className="space-y-1">
            <div className="flex items-center justify-between text-xs">
              <span className="text-slate-300 font-medium flex items-center gap-1.5"><span>{step.icon}</span>{step.label}</span>
              <div className="flex items-center gap-3">
                <span className="text-white font-bold tabular-nums">{formatNum(step.value)}</span>
                {i > 0 && <span className={`font-bold tabular-nums text-xs ${rateColor(r)}`}>{r}%</span>}
              </div>
            </div>
            <div className="h-7 rounded-lg overflow-hidden bg-slate-800/60">
              <div className={`h-full rounded-lg bg-gradient-to-r ${barColor(i === 0 ? 100 : r)} transition-all duration-700`} style={{ width: `${width}%` }} />
            </div>
          </div>
        );
      })}
...
function SalesTickerBanner({ ranking }: { ranking: RankRow[] }) {
  const items = ranking.filter(r => r.vendas > 0)
    .map(r => `🏆 ${r.nome.split(" ")[0]} — ${r.vendas} venda${r.vendas === 1 ? "" : "s"} · ${r.pontos} pts`);
...
function MetaAtingidaOverlay({ show, onDone }: { show: boolean; onDone: () => void }) {
  useEffect(() => {
    if (show) { const t = setTimeout(onDone, 5000); return () => clearTimeout(t); }
  }, [show, onDone]);
  if (!show) return null;
  return (
    <div className="fixed inset-0 pointer-events-none z-50 flex items-center justify-center overflow-hidden">
      ...
      <div className="text-center" style={{ animation: "fadeInUp .5s ease-out" }}>
        <div className="text-5xl font-black text-emerald-400 mb-3" style={{ textShadow: "0 0 40px rgba(16,185,129,0.8), 0 0 80px rgba(16,185,129,0.4)" }}>
          🎯 META ATINGIDA!
        </div>
...
function PerformanceTVPage() {
  ...
  const profQ = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("profiles").select("id, nome, avatar_url, foto_url, ativo").eq("ativo", true);
      return data ?? [];
    },
  });
  ...
  const totaisMesPrev = useMemo(() => rankingMesPrev.reduce((acc, r) => ({
    ...
    visitas: acc.visitas + r.visitas,
    leads: acc.leads + r.leads,
  }), { vendas: 0, visitas: 0, leads: 0 }), [rankingMesPrev]);
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-blue-950 text-white -m-6 ${isFullscreen ? "overflow-hidden" : "pb-14"}`}>
      <style>{TV_STYLES}</style>
      <MetaAtingidaOverlay show={showCelebration} onDone={() => setShowCelebration(false)} />

      {/* Header */}
      <div className="border-b border-slate-800/50 bg-gradient-to-r from-slate-900/95 via-slate-900/90 to-blue-950/80 backdrop-blur-sm sticky top-0 z-40">
          ...
          <div className="flex items-center justify-between gap-3 flex-wrap">
            ...
            <div className="flex-1 flex justify-center min-w-[300px]">
              <Tabs value={activeTab} onValueChange={(v) => { setActiveTab(v as TabType); setAutoRotate(false); }}>
                <TabsList className="bg-slate-800/60 border border-slate-700/50">
                  <TabsTrigger value="realxmeta" className="data-[state=active]:bg-purple-600 data-[state=active]:text-white text-slate-300 text-xs sm:text-sm">
                    <Target className="w-4 h-4 mr-1.5" /> Real x Meta
                  </TabsTrigger>
                  <TabsTrigger value="vendas" className="data-[state=active]:bg-blue-600 data-[state=active]:text-white text-slate-300 text-xs sm:text-sm">
                    <Trophy className="w-4 h-4 mr-1.5" /> Vendas
                  </TabsTrigger>
                  <TabsTrigger value="produtividade" className="data-[state=active]:bg-emerald-600 data-[state=active]:text-white text-slate-300 text-xs sm:text-sm">
            ...
            <div className="flex items-center gap-3 shrink-0">
              ...
              <div className="flex items-center gap-1.5">
                <Button variant="outline" size="sm" onClick={() => setAutoRotate(v => !v)}
                ...
                <Button variant="outline" size="icon" onClick={refetchAll}
                  className="bg-slate-800/50 border-slate-700/50 text-white hover:bg-slate-700/50 hover:text-white" title="Atualizar">
                  <RefreshCw className={`h-4 w-4 ${isLoading ? "animate-spin" : ""}`} />
                </Button>
                <Button variant="outline" size="icon" onClick={toggleFullscreen}
                  className="bg-slate-800/50 border-slate-700/50 text-white hover:bg-slate-700/50 hover:text-white" title={isFullscreen ? "Sair" : "Tela cheia"}>
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-48 max-h-80 overflow-y-auto">
                  {[2026, 2025, 2024].map(ano => (
                    <div key={ano}>
                      <DropdownMenuSeparator />
                      <div className="px-2 py-1 text-xs font-bold text-slate-500">{ano}</div>
                      {MESES.map((m, idx) => (
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="outline" className="gap-2 bg-slate-800/50 border-slate-700/50 text-white hover:bg-slate-700/50 hover:text-white min-w-[180px] justify-between">
                    <span className="flex items-center gap-2"><Calendar className="h-4 w-4" /> {periodLabels[periodo]}</span>
                    <ChevronDown className="h-4 w-4" />
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end" className="w-48">
                  {(Object.entries(periodLabels) as [PeriodOption, string][]).map(([k, label]) => (
                    <DropdownMenuItem key={k} onClick={() => setPeriodo(k)} className={periodo === k ? "bg-accent" : ""}>{label}</DropdownMenuItem>
                  ))}
                </DropdownMenuContent>
              </DropdownMenu>
            )}
          ...
          <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-5">
              ...
              <div className="bg-slate-900/60 rounded-2xl border border-slate-800/50 p-5">
                ...
                <div className="flex flex-col items-center gap-4">
                  <GaugeChart percentage={pctAtingimento} label="% Atingido" metaDefined={metaDefined} />
                  <div className="w-full grid grid-cols-2 gap-3 text-center">
                    <div className="bg-slate-800/50 rounded-xl p-3">
                      <div className="text-xs text-slate-400 mb-1">Realizado</div>
                      <div className="text-lg font-bold text-purple-300 tabular-nums">{formatNum(totaisMes.vendas)}</div>
                    </div>
                    <div className="bg-slate-800/50 rounded-xl p-3">
                      <div className="text-xs text-slate-400 mb-1">Meta</div>
                      <div className="text-lg font-bold text-slate-300 tabular-nums">{metaDefined ? formatNum(metaTotais.vendas) : "—"}</div>
                    </div>
                    <div className="bg-slate-800/50 rounded-xl p-3">
                      <div className="text-xs text-slate-400 mb-1">Gap</div>
                      <div className={`text-base font-bold tabular-nums ${metaTotais.vendas - totaisMes.vendas > 0 ? "text-red-400" : "text-emerald-400"}`}>
                        {metaDefined ? `${metaTotais.vendas - totaisMes.vendas > 0 ? "-" : "+"}${formatNum(Math.abs(metaTotais.vendas - totaisMes.vendas))}` : "—"}
                      </div>
                    </div>
                    <div className="bg-slate-800/50 rounded-xl p-3">
                      <div className="text-xs text-slate-400 mb-1">Tendência</div>
                      <div className={`text-base font-bold tabular-nums ${tendencia >= 100 ? "text-emerald-400" : tendencia >= 70 ? "text-amber-400" : "text-red-400"}`}>
              ...
              <div className="bg-slate-900/60 rounded-2xl border border-slate-800/50 p-5 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <KPICard label="Contratos" numericValue={totaisMes.vendas} icon={FileCheck} variant="success" delta={deltaVendas} />
                  <KPICard label="Leads" numericValue={totaisMes.leads} icon={Users} variant="info" />
                  <KPICard label="Agendamentos" numericValue={totaisMes.agendamentos} icon={CalendarCheck} variant="accent" />
                  <KPICard label="Visitas" numericValue={totaisMes.visitas} icon={Eye} variant="warning" />
                </div>
              </div>
              ...
              <div className="bg-slate-900/60 rounded-2xl border border-slate-800/50 p-5">
                ...
                <RankingLateral ranking={rankingMes.filter(r => r.vendas > 0 || r.pontos > 0).slice(0, 10)} type="vendas" />
              </div>
            </div>
            ...
            <div className="bg-slate-900/60 rounded-2xl border border-slate-800/50 p-5">
              ...
              {(() => {
                ...
                if (rows.length === 0) return <p className="text-sm text-slate-500 text-center py-6">Sem dados para o mês</p>;
                      ...
                      return (
                        <div key={r.corretorId} className="space-y-1">
                          <div className="flex justify-between text-xs">
                            <span className="text-slate-300 font-medium truncate max-w-[200px]">{r.nome}</span>
                            <div className="flex items-center gap-3">
                              {r.meta > 0 && <span className="text-slate-500 tabular-nums">Meta: {r.meta}</span>}
                              <span className="text-white font-semibold tabular-nums">{r.vendas}</span>
                            </div>
                          </div>
                          <div className="h-5 bg-slate-800/60 rounded-md overflow-hidden relative">
                            <div className="h-full rounded-md bg-gradient-to-r from-purple-600 to-purple-400 transition-all duration-700"
                              style={{ width: `${Math.max(valW, 2)}%` }} />
                            {metaW > 0 && (
                              <div className="absolute top-0 bottom-0 w-0.5 bg-white/60" style={{ left: `${metaW}%` }} />
                            )}
                          </div>
                        </div>
                      );
        ...
        {activeTab === "vendas" && (
          <>
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3 mb-5">
              <KPICard label="Vendas" numericValue={totaisPeriodo.vendas} icon={Trophy} variant="success" highlight />
              <KPICard label="Visitas" numericValue={totaisPeriodo.visitas} icon={Eye} variant="warning" />
              <KPICard label="Agendamentos" numericValue={totaisPeriodo.agendamentos} icon={CalendarCheck} variant="accent" />
              <KPICard label="Documentação" numericValue={totaisPeriodo.documentacoes} icon={FileCheck} variant="info" />
              <KPICard label="Leads" numericValue={totaisPeriodo.leads} icon={Users} variant="default" />
              <KPICard label="Corretores" numericValue={rankingProd.filter(r => r.pontos > 0).length} icon={Users} variant="default" />
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-5">
              ...
              <div className="bg-slate-900/60 rounded-2xl border border-slate-800/50 p-5">
                ...
                <RankingLateral ranking={rankingProd.filter(r => r.vendas > 0).sort((a, b) => b.vendas - a.vendas).slice(0, 15)} type="vendas" />
              </div>
            </div>

            <div className="bg-slate-900/60 rounded-2xl border border-slate-800/50 p-5">
              <h3 className="text-xs text-slate-400 uppercase tracking-wider mb-3 flex items-center gap-2">
                <TrendingUp className="w-4 h-4 text-cyan-400" /> Funil de Conversão — {periodLabels[periodo]}
              </h3>
              <FunilConversao
        ...
        {activeTab === "produtividade" && (
          <>
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3 mb-5">
              <KPICard label="Pontos" numericValue={totaisPeriodo.pontos} icon={Star} variant="accent" highlight />
              <KPICard label="Ligações" numericValue={totaisPeriodo.ligacoes} icon={Phone} variant="info" />
              <KPICard label="WhatsApp" numericValue={totaisPeriodo.whatsapp} icon={MessageSquare} variant="accent" />
              <KPICard label="Agendamentos" numericValue={totaisPeriodo.agendamentos} icon={CalendarCheck} variant="default" />
              <KPICard label="Visitas" numericValue={totaisPeriodo.visitas} icon={Eye} variant="warning" />
              <KPICard label="Documentação" numericValue={totaisPeriodo.documentacoes} icon={FileCheck} variant="info" />
              <KPICard label="Vendas" numericValue={totaisPeriodo.vendas} icon={Trophy} variant="success" />
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-5">
              ...
              <div className="bg-slate-900/60 rounded-2xl border border-slate-800/50 p-5">
                ...
                <RankingLateral ranking={rankingProd.filter(r => r.pontos > 0).slice(0, 15)} type="pontos" positionChanges={positionChanges} />
              </div>
            </div>
            ...
            <div className="bg-slate-900/60 rounded-2xl border border-slate-800/50 p-5">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="text-slate-400 border-b border-slate-700/50 text-[11px] uppercase tracking-wider">
                  ...
                  <tbody>
                    {rankingProd.filter(r => r.pontos > 0).map((r, idx) => (
                      <tr key={r.corretorId} className="border-b border-slate-800/30 hover:bg-slate-800/20 transition-colors">
                        <td className="py-2.5 px-3 text-slate-400 font-bold tabular-nums">{idx + 1}</td>
                        <td className="py-2.5 px-3">
                          <div className="flex items-center gap-2">
                            <Avatar className="w-7 h-7">
                              <AvatarImage src={r.foto ?? undefined} />
                              <AvatarFallback className="text-[10px] bg-slate-700 text-white">{getInitials(r.nome)}</AvatarFallback>
                            </Avatar>
                            <span className="text-white font-medium">{r.nome}</span>
                          </div>
                        </td>
                        <td className={`text-center py-2 px-2 tabular-nums font-semibold ${getHeatColor(r.ligacoes, heatCols.lig)}`}>{r.ligacoes || "—"}</td>
                        <td className={`text-center py-2 px-2 tabular-nums font-semibold ${getHeatColor(r.whatsapp, heatCols.wpp)}`}>{r.whatsapp || "—"}</td>
                        <td className={`text-center py-2 px-2 tabular-nums font-semibold ${getHeatColor(r.agendamentos, heatCols.agd)}`}>{r.agendamentos || "—"}</td>
                        <td className={`text-center py-2 px-2 tabular-nums font-semibold ${getHeatColor(r.visitas, heatCols.vis)}`}>{r.visitas || "—"}</td>
                        <td className={`text-center py-2 px-2 tabular-nums font-semibold ${getHeatColor(r.documentacoes, heatCols.doc)}`}>{r.documentacoes || "—"}</td>
                        <td className={`text-center py-2 px-2 tabular-nums font-semibold ${getHeatColor(r.vendas, heatCols.ven)}`}>{r.vendas || "—"}</td>
                        <td className="text-right py-2 px-3 tabular-nums font-bold text-cyan-300">{formatNum(r.pontos)}</td>
                      </tr>
                    ))}
                    {rankingProd.filter(r => r.pontos > 0).length === 0 && (
                      <tr><td colSpan={9} className="py-8 text-center text-slate-500">Sem atividade no período</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
