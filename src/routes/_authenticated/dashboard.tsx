...
import {
  computeAgentMetrics, rankAgents, progressoMeta, MESES_PT,
} from "@/lib/metas";
...
function DashboardPage() {
  ...
  const metrics = useMemo(
    () => computeAgentMetrics(leadsQ.data ?? [], agendamentosQ.data ?? [], ano, mes),
    [leadsQ.data, agendamentosQ.data, ano, mes],
  );
  ...
  const totais = useMemo(() => {
    let leads_total = 0, leads_atendidos = 0, visitas = 0, vendas = 0;
  ...
  const visao = canSeeAll ? totais : (minhasMetricas ?? {
    leads_total: 0, leads_atendidos: 0, visitas: 0, vendas: 0, taxa_conversao: 0,
  });
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <KpiCard icon={<Users className="h-5 w-5" />} label="Leads recebidos" value={visao.leads_total} />
        <KpiCard icon={<TrendingUp className="h-5 w-5" />} label="Atendidos" value={visao.leads_atendidos} />
        <KpiCard icon={<CheckCircle2 className="h-5 w-5" />} label="Visitas realizadas" value={visao.visitas} />
      ...
      {meta && (
        <Card>
          ...
          <CardContent className="grid md:grid-cols-3 gap-4">
            <MetaProgresso label="Atendimentos" atual={visao.leads_atendidos} meta={meta.meta_leads_atendidos} />
            ...
            {ranking.length === 0 ? (
              ...
              <ol className="space-y-2">
                {ranking.slice(0, 10).map((r) => (
                  <li key={r.corretor_id} className="flex items-center gap-3 p-2 rounded-md hover:bg-muted/50">
  ...
  return (
    <Card className={accent ? "border-primary" : ""}>
      <CardContent className="p-4 flex items-center gap-3">
        <div className={`h-10 w-10 rounded-md flex items-center justify-center ${accent ? "bg-primary text-primary-foreground" : "bg-muted"}`}>{icon}</div>
        <div>
  ...
  return (
    <div>
      <div className="flex justify-between text-sm mb-1">
        ...
        <span className="font-medium">{atual} / {meta}</span>
      </div>
      <Progress value={p} />
