      ...
      <CardContent className="space-y-3">
        {!supported ? (
          <div className="text-sm text-muted-foreground">
            Seu navegador não suporta notificações push. Use Chrome/Edge no Android ou Safari 16.4+
            no iOS.
          </div>
        ) : isIosNotInstalled ? (
          ...
          <div className="text-sm text-muted-foreground">
            Você bloqueou as notificações. Para reativar, vá nas configurações do navegador para
            este site e permita notificações.
          </div>
        ) : subscribed ? (
          <>
            <div className="text-sm text-muted-foreground">
              Notificações ativas neste aparelho. Você recebe push de novos leads, agendamentos e
              tarefas.
            </div>
            <div className="flex justify-end">
              <Button variant="outline" size="sm" disabled={loading} onClick={handleUnsubscribe}>
                <BellSlash className="mr-2 h-4 w-4" /> Desativar
              </Button>
            </div>
          </>
        ) : (
          <>
            <div className="text-sm text-muted-foreground">
              Receba alertas em tempo real de novos leads atribuídos, tarefas e agendamentos
              próximos — mesmo com o app fechado.
            </div>
            <div className="flex justify-end">
