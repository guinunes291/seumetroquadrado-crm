-- ============================================================================
-- Evolução do CRM (PR #69) — migrações consolidadas, v2 (ordem de aplicação).
-- Cole TUDO no SQL editor do Supabase e clique em Run.
--
-- v2: adiciona a seção [0/19] com pré-requisitos de 16/06 que nunca chegaram
-- ao banco vivo (produtividade/pontuação e tabelas do funil). A v1 falhava em
-- "relation public.atividades_diarias does not exist" por causa disso.
--
-- Contexto: o código do PR #69 já está no ar, mas estas migrações nunca foram
-- aplicadas ao banco vivo — por isso o app falha ao chamar RPCs como
-- pipeline_snapshot_v2, transicionar_lead, aprovar_venda, salvar_modo_visita.
--
-- O script roda numa única transação: se algo falhar, nada é aplicado
-- (foi o que aconteceu na v1 — o banco ficou intacto).
-- Antes de rodar: faça um backup (Database > Backups).
-- ============================================================================

-- ============================================================================
-- [0/19] PRÉ-REQUISITOS — objetos de migrações de 16/06 que nunca chegaram ao
-- banco vivo e são exigidos pelas migrações de 11/07 abaixo.
-- Todos os comandos são idempotentes (IF NOT EXISTS / OR REPLACE / DROP+CREATE).
-- Fonte: 20260616130000 (produtividade) e 20260616130200 (visitas/propostas).
-- ============================================================================

-- ---------------------------------------------------------------------------
-- 0.1) set_updated_at — cria apenas se não existir (não substitui a versão
--      que já está no banco, usada por dezenas de triggers).
-- ---------------------------------------------------------------------------
DO $do$
BEGIN
...
-- ---------------------------------------------------------------------------
-- 0.2) Configuração de pontos por atividade + função pontos_de.
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.configuracao_pontuacao (
...
DROP POLICY IF EXISTS "todos leem config pontuacao" ON public.configuracao_pontuacao;
CREATE POLICY "todos leem config pontuacao" ON public.configuracao_pontuacao
  FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "gestor gerencia config pontuacao" ON public.configuracao_pontuacao;
...
-- ---------------------------------------------------------------------------
-- 0.3) Atividades diárias por corretor + bump_atividade.
-- ---------------------------------------------------------------------------
...
CREATE INDEX IF NOT EXISTS idx_atividades_diarias_dia ON public.atividades_diarias(dia DESC);
CREATE INDEX IF NOT EXISTS idx_atividades_diarias_corretor ON public.atividades_diarias(corretor_id, dia DESC);
...
DROP POLICY IF EXISTS "corretor ve suas atividades" ON public.atividades_diarias;
...
-- ---------------------------------------------------------------------------
-- 0.4) Triggers de pontuação em interações/agendamentos/transições.
--      (trg_pont_venda NÃO é criado: a migração de aprovação de vendas, na
--      seção 4, descontinua esse gatilho e passa a materializar venda/VGV
--      apenas na aprovação. Ela também redefine pont_after_transicao.)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 0.5) Metas diárias e alertas de produtividade (complementos do módulo).
-- ---------------------------------------------------------------------------
...
DROP POLICY IF EXISTS "corretor ve sua meta diaria" ON public.metas_diarias;
CREATE POLICY "corretor ve sua meta diaria" ON public.metas_diarias
  FOR SELECT TO authenticated
  USING (corretor_id = auth.uid()
         OR public.has_role(auth.uid(),'admin') OR public.has_role(auth.uid(),'gestor') OR public.has_role(auth.uid(),'superintendente'));
DROP POLICY IF EXISTS "gestor gerencia metas diarias" ON public.metas_diarias;
...
CREATE TABLE IF NOT EXISTS public.alertas_produtividade (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  corretor_id uuid NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_alertas_prod_corretor ON public.alertas_produtividade(corretor_id, created_at DESC);
...
DROP POLICY IF EXISTS "corretor ve seus alertas prod" ON public.alertas_produtividade;
CREATE POLICY "corretor ve seus alertas prod" ON public.alertas_produtividade
  FOR SELECT TO authenticated
  USING (corretor_id = auth.uid()
         OR public.has_role(auth.uid(),'admin') OR public.has_role(auth.uid(),'gestor') OR public.has_role(auth.uid(),'superintendente'));
DROP POLICY IF EXISTS "corretor marca seus alertas prod" ON public.alertas_produtividade;
...
-- ---------------------------------------------------------------------------
-- 0.6) Tabelas do funil que faltam no banco vivo: visitas, propostas e
--      propostas_visitantes. As policies de carteira são criadas na seção 6
--      (related_lead_rls) — aqui só estrutura, grants e RLS ligado.
-- ---------------------------------------------------------------------------
...
CREATE INDEX IF NOT EXISTS idx_visitas_corretor ON public.visitas(corretor_id, data_visita DESC);
...
CREATE INDEX IF NOT EXISTS idx_propostas_corretor ON public.propostas(corretor_id, created_at DESC);
...
CREATE TABLE IF NOT EXISTS public.propostas_visitantes (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  nome text NOT NULL,
  telefone text,
  email text,
...
-- ---------------------------------------------------------------------------
-- 0.7) Garantia: funções de push usadas pelos REVOKEs da seção 2. Elas são
--      funções de trigger (não aparecem no typegen) e devem existir desde
--      junho; este bloco só as recria se realmente faltarem.
-- ---------------------------------------------------------------------------
DO $do$
BEGIN
  IF to_regprocedure('public.push_lead_distribuido()') IS NULL THEN
    EXECUTE $f$
      CREATE FUNCTION public.push_lead_distribuido()
    ...
    $f$;
  END IF;
  IF to_regprocedure('public.push_tarefa_criada()') IS NULL THEN
    EXECUTE $f$
      CREATE FUNCTION public.push_tarefa_criada()
      RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $body$
      BEGIN
...
-- ============================================================================
-- [1/19] 20260711120000_invite_only_lead_access.sql
-- ============================================================================
...
-- ============================================================================
-- [2/19] 20260711121000_push_outbox_claim.sql
-- ============================================================================
...
-- ============================================================================
-- [3/19] 20260711121500_documentacao_server_mediation.sql
-- ============================================================================
...
-- ============================================================================
-- [4/19] 20260711122000_sales_approval_integrity.sql
-- ============================================================================
...
-- ============================================================================
-- [5/19] 20260711123000_invite_operations.sql
-- ============================================================================
...
-- ============================================================================
-- [6/19] 20260711123500_related_lead_rls.sql
-- ============================================================================
...
-- ============================================================================
-- [7/19] 20260711124000_scale_read_models_v2.sql
-- ============================================================================
...
-- ============================================================================
-- [8/19] 20260711125000_api_clientes.sql
-- ============================================================================
...
-- ============================================================================
-- [9/19] 20260711126000_landing_webhook_hardening.sql
-- ============================================================================
...
-- ============================================================================
-- [10/19] 20260711127000_atendimento_inbox_v2.sql
-- ============================================================================
...
-- ============================================================================
-- [11/19] 20260711130000_lead_status_transition_guard.sql
-- ============================================================================
...
-- ============================================================================
-- [12/19] 20260711131000_samiq_governance.sql
-- ============================================================================
...
-- ============================================================================
-- [13/19] 20260711132000_vitrine_publica.sql
-- ============================================================================
...
-- ============================================================================
-- [14/19] 20260711133000_modo_visita.sql
-- ============================================================================
...
-- ============================================================================
-- [15/19] 20260711134000_fechamento_sinais_calibrados.sql
-- ============================================================================
...
-- ============================================================================
-- [16/19] 20260711135000_projetos_vitrine_rich_media.sql
-- ============================================================================
...
-- ============================================================================
-- [17/19] 20260711136000_projetos_webhook_token_lockdown.sql
-- ============================================================================
...
-- ============================================================================
-- [18/19] 20260711137000_vitrine_rollout_upgrade.sql
-- ============================================================================
