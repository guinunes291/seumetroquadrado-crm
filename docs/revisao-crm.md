...
| **6** | Empreendimento comercial + relatórios (ligações/WA/tempo de resposta) + consolidar menu | munição + clareza | médio-alto | média | ✅ **munição comercial** (aba Comercial) · **menu consolidado** por intenção (sidebar) · **relatório de atividade** por corretor (ligações/WhatsApp/visitas/total) no Painel do Gestor. Resta tempo de resposta médio e a fusão Dashboard+Meu Dia. |
