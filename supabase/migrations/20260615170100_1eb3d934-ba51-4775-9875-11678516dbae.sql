-- Copa SMQ (Fase 4) — funções (RPCs). Pontuação calculada on-the-fly do CRM
-- (agendamentos + lead_status_transitions), mais bônus manual de copa_pontuacoes.
-- Mutations exigem admin/gestor (padrão do distribuir_lead).

-- Pontos de 1 corretor numa janela (auto, sem bônus manual). Usada pela apuração.
...
-- Ranking da edição (auto + bônus manual), ordenado por total desc, nome asc.
...
-- Define a lista de participantes (desativa quem saiu, reativa/insere selecionados).
...
-- Sorteio: atribui seleção (round-robin embaralhado) e cria confrontos da fase de grupos.
...
BEGIN
  ...
  -- Atribui seleção aleatória (round-robin sobre seleções embaralhadas).
  WITH parts AS (
  ...
  -- Pareia participantes embaralhados (1-2, 3-4, ...).
  FOR _r IN
...
-- Apura (auto) os vencedores de uma fase por pontos na janela, exceto os definidos manualmente.
...
-- Override manual do vencedor de um confronto.
