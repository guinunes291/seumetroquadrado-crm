-- leads_filtered_v5 / leads_status_counts_v5 — filtro por resultado da
-- ANÁLISE DE CRÉDITO (pedido do dono, 2026-08-11; casa com o item 3.1).
--
-- O que muda em relação à v4 (20260808123000):
--  * Novo `_analise text`: recorte pela ÚLTIMA análise de crédito do lead —
--    'aprovada' | 'aprovada_condicionada' | 'reprovada' | 'em_andamento'
--    (enviada/pendente). Validado na entrada (valor desconhecido = ERRO
--    22023, lição do fim do ELSE true); NULL/'all' desliga e a subconsulta
--    correlata só corre com o filtro ativo (índice lead_id+created_at DESC
--    da 20260810120000).
--  * Compõe por AND com todos os demais recortes, como sempre.
--
-- Regra de ouro mantida: o recorte existe nas DUAS funções (lista e
-- contagens) — chips e lista nunca divergem. NOME NOVO de propósito
-- (assinatura nova em RPC viva quebra o PostgREST); a v4 permanece como
-- degrau do rpcWithFallback (v5 → v4 → v3 → …).
...
-- Contagens por status no MESMO escopo e com os MESMOS recortes da lista v5
