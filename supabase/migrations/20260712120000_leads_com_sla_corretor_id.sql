-- Adiciona corretor_id ao retorno de leads_com_sla.
--
-- A Central de Comando ("Hoje") ganhou visão de operação: admin/superintendente
-- veem tudo, gestor vê a própria equipe. Para escopar o SLA por equipe no
-- cliente é preciso saber de quem é cada lead — e a função não devolvia isso.
-- Esta migration só ACRESCENTA a coluna corretor_id; toda a lógica de SLA
-- (prazo por origem/webhook, temperatura, escopo por papel) é idêntica à de
-- 20260705130000. Como o tipo de retorno muda, é DROP + CREATE. Backward-
-- compatível: os consumidores atuais (Kanban, Blitz, detalhe do lead, Hoje)
-- ignoram a coluna extra.
DROP FUNCTION IF EXISTS public.leads_com_sla(uuid);
CREATE FUNCTION public.leads_com_sla(_corretor uuid DEFAULT NULL)
RETURNS TABLE (
  lead_id uuid,
  corretor_id uuid,
  nome text,
