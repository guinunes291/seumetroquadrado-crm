...
BEGIN
  ...
  FOR _r IN
    SELECT la, lb, lv, wo, sem, pos FROM (
      VALUES
      (39782610::bigint,38431527::bigint,39782610::bigint,false,1,1),
  ...
  LOOP
    ...
    IF _aid IS NULL OR (NOT _r.wo AND _bid IS NULL) THEN
      ...
      IF _aid IS NULL THEN _missing := array_append(_missing, _r.la::text); END IF;
      IF NOT _r.wo AND _bid IS NULL THEN _missing := array_append(_missing, _r.lb::text); END IF;
      CONTINUE;
    END IF;
