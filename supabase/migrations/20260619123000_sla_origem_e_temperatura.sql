-- Parte 3 (C1 + C2): SLA por origem e score de "quente".

-- C1: SLA de primeiro contato por origem — Facebook Ads = 5 min (demais já são 30).
UPDATE public.distribuicao_config SET sla_minutos = 5 WHERE origem = 'facebook';

-- C1: leads_com_sla agora retorna nome/telefone/status (para o "Meu Dia" renderizar
-- o bloco "SLA estourando" sem uma segunda query). Mantém o cálculo de SLA por
-- origem (LEFT JOIN distribuicao_config) e atualiza a temperatura calculada (C2).
...
-- C2: temperatura ("quente") considera também etapa avançada e cadastro recente
-- com interação, além da recência da última interação. (cron recalc-temperatura */10).
