-- leads_filtered_v4 / leads_status_counts_v4 — filtro "parado há X dias" e o
-- fim do ELSE true (estrategia-2026-08 item 2.11 / auditoria ux-ia-2026-08).
--
-- O que muda em relação à v3 (20260728100000):
--  * `_contato` desconhecido deixava de filtrar EM SILÊNCIO (ELSE true nas
--    duas funções): a tela usada para decidir quem cobrar mostrava a lista
--    inteira como se fossem os parados. Agora o valor é validado na entrada
--    e valor desconhecido é ERRO (22023) — alto e cedo, nunca errado e mudo.
--  * Novo `_parado_dias int`: o recorte "parado há X+ dias" vira paramétrico
--    (1..365) em vez de aceitar só as réguas fixas de 5 e 30 dias. Os valores
--    `sem_contato_5d`/`sem_contato_30d` de `_contato` continuam aceitos por
--    compatibilidade (visões salvas/URLs antigas). Mesmo predicado dos
--    recortes fixos: sem interação há X+ dias e fora dos status terminais.
--  * `_parado_dias` compõe com `_contato` por AND (ex.: "com follow-up" E
--    "parado 15d+"), como os demais filtros da base.
--
-- Regra de ouro mantida: TODO recorte existe nas DUAS funções (lista e
-- contagens), senão os chips divergem da lista (lição da 20260720120000).
-- NOME NOVO de propósito (lição da v2→v3): assinatura nova em RPC viva quebra
-- o PostgREST; a v3 permanece como degrau do rpcWithFallback (v4 → v3 → …).

CREATE OR REPLACE FUNCTION public.leads_filtered_v4(
...
REVOKE ALL ON FUNCTION public.leads_filtered_v4(boolean, text, text, text, text, timestamptz, timestamptz, text, text, text, int, text, text, int, int) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.leads_filtered_v4(boolean, text, text, text, text, timestamptz, timestamptz, text, text, text, int, text, text, int, int) FROM anon;
GRANT EXECUTE ON FUNCTION public.leads_filtered_v4(boolean, text, text, text, text, timestamptz, timestamptz, text, text, text, int, text, text, int, int) TO authenticated;

-- Contagens por status no MESMO escopo e com os MESMOS recortes da lista v4
-- (validação estrita + _parado_dias) — chips e lista nunca divergem.
CREATE OR REPLACE FUNCTION public.leads_status_counts_v4(
  _na_lixeira boolean DEFAULT false,
  _origem text DEFAULT NULL,
  _corretor text DEFAULT NULL,
  _temperatura text DEFAULT NULL,
...
REVOKE ALL ON FUNCTION public.leads_status_counts_v4(boolean, text, text, text, timestamptz, timestamptz, text, text, text, int) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.leads_status_counts_v4(boolean, text, text, text, timestamptz, timestamptz, text, text, text, int) FROM anon;
GRANT EXECUTE ON FUNCTION public.leads_status_counts_v4(boolean, text, text, text, timestamptz, timestamptz, text, text, text, int) TO authenticated;
