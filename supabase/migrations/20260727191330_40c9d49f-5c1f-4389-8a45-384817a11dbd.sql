CREATE OR REPLACE FUNCTION public.atribuir_oferta_ativa(_oferta_id uuid, _corretor_ids uuid[])
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT * INTO _oferta FROM public.ofertas_ativas WHERE id = _oferta_id FOR UPDATE;
  IF NOT FOUND THEN
  ...
  -- Trava TODOS os leads da lista de uma vez, sempre na mesma ordem (id ASC).
  -- Sem isso, as rotinas automáticas de distribuição/SLA travam as mesmas
  -- linhas em outra ordem e o Postgres aborta com "deadlock detected".
  PERFORM 1
    FROM public.leads l
   WHERE l.id IN (SELECT lead_id FROM public.oferta_ativa_leads WHERE oferta_id = _oferta_id)
   ORDER BY l.id
     FOR UPDATE;
  ...
  FOR _i IN 1.._n LOOP
    ...
    UPDATE public.leads
       SET corretor_id = _cid,
           ...
           data_distribuicao = now(),
           updated_at = now()
     WHERE id IN (SELECT lid FROM unnest(_slice) AS lid ORDER BY lid);
