-- Atendimento inbox v3: a fila que faltava — "novos" (primeiro contato).
--
-- Bug de propósito da tela: a v2 só classificava lead com sinal JÁ registrado
-- (interação de entrada, follow-up vencido, esfriando com ultima_interacao,
-- docs pendentes). Um lead recém-distribuído não tem nada disso — status
-- novo/aguardando_atendimento, zero interações, sem follow-up — e ficava
-- invisível em TODAS as filas. Resultado: o badge da sidebar (nav_pendencias)
-- contava a fila de entrada, mas a tela de Atendimento abria "zerada" e o
-- corretor nunca teve leads ali para trabalhar. A v3 corrige:
--
--   1. Fila "novos" (1ª na classificação e na tela): lead em
--      novo/aguardando_atendimento — chegou para o corretor e o SLA do
--      primeiro contato está correndo. Mais antigo primeiro no desempate.
--   2. Régua única de esfriamento: dias sem contato passa a usar
--      COALESCE(ultima_interacao, ultimo_contato, created_at) — a mesma régua
--      de lead-flags/metrics. Lead quente já em atendimento mas sem interação
--      REGISTRADA também esfria (antes o NULL o excluía da fila para sempre).
--
-- A v2 permanece intacta: é o fallback do cliente (rpcWithFallback/PGRST202)
-- enquanto esta migration não chega ao ambiente.

CREATE OR REPLACE FUNCTION public.atendimento_inbox_v3(
...
REVOKE ALL ON FUNCTION public.atendimento_inbox_v3(uuid, integer)
  FROM PUBLIC, anon, service_role;
GRANT EXECUTE ON FUNCTION public.atendimento_inbox_v3(uuid, integer)
  TO authenticated;

COMMENT ON FUNCTION public.atendimento_inbox_v3(uuid, integer)
  IS 'Inbox v3: fila novos (primeiro contato) + regua unica de esfriamento; contagens completas e no maximo 30 itens por fila, restrita ao corretor autorizado.';
