...
-- 5) `valor_imovel_simulacao` guarda o "valor do imóvel" da carta só como
--    REFERÊNCIA: o analista simula no teto da faixa MCMV, não no imóvel do
--    cliente. O encaixe em produto usa sempre o "a partir de" do catálogo.
-- 6) `poder_compra` é coluna GERADA (financiamento + FGTS + subsídio +
...
ALTER TABLE public.analises_credito
  ...
  ADD COLUMN IF NOT EXISTS valor_imovel_simulacao numeric(12,2),
...
BEGIN
  ALTER TABLE public.analises_credito
    ...
    CHECK (
      ...
      AND COALESCE(valor_imovel_simulacao, 0) >= 0
