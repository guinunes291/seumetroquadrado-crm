-- Patch de dados de produção (confrontos da final da Copa) com UUIDs fixos.
-- Guarda de replay: só executa se a fase referenciada existir (em ambiente
-- limpo os registros não existem e o patch é um no-op).
DO $patch$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.copa_fases
                 WHERE id = '3dd9757e-00a9-4037-80af-09fcdcb2e94c') THEN
    RETURN;
  END IF;
