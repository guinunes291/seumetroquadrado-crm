-- Filas por zona: cada corretor recebe apenas leads da(s) zona(s) que atende.

CREATE OR REPLACE FUNCTION public.zona_normalizar(_txt text)
RETURNS text LANGUAGE sql IMMUTABLE AS $$
  SELECT CASE
    WHEN v IS NULL OR v = '' THEN NULL
    WHEN v IN ('zn') THEN 'Norte'
    WHEN v IN ('zs') THEN 'Sul'
    WHEN v IN ('zl') THEN 'Leste'
    WHEN v IN ('zo') THEN 'Oeste'
    WHEN v LIKE '%centro%' OR v LIKE '%central%' THEN 'Centro'
    WHEN v LIKE '%norte%' THEN 'Norte'
    WHEN v LIKE '%sul%' THEN 'Sul'
    WHEN v LIKE '%leste%' THEN 'Leste'
    WHEN v LIKE '%oeste%' THEN 'Oeste'
    ELSE NULL
  END
  FROM (SELECT lower(btrim(translate(coalesce(_txt,''),
    'ÀÁÂÃÄÉÊÍÓÔÕÚÇàáâãäéêíóôõúç','AAAAAEEIOOOUCaaaaaeeiooouc'))) AS v) s
$$;

ALTER TABLE public.leads ADD COLUMN IF NOT EXISTS zona text;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS zonas text[] NOT NULL DEFAULT '{}'::text[];

-- Zona do lead: o campo do próprio lead manda; sem ele, a zona do projeto.
...
GRANT EXECUTE ON FUNCTION public.zona_normalizar(text) TO authenticated, service_role;
...
BEGIN
  ...
  VALUES
    (_lead_id, _vencedor, _tipo,
     ...
     END,
     _distribuido_por, _slug, _regra, 'ok')
  RETURNING id INTO _log_id;
  INSERT INTO public.distribuicao_log_contexto (log_id, contexto) VALUES (_log_id, _contexto);
