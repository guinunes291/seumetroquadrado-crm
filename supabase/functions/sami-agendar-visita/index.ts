...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    return json({ ok: false, erro: "body_invalido", detalhe: "JSON inválido" });
  }
  ...
  if (!dataHora || isNaN(quando.getTime()))
    return json({
      ok: false,
      erro: "data_invalida",
      detalhe: "use ISO 8601 com fuso, ex.: 2026-07-09T15:00:00-03:00",
    });
  if (quando.getTime() < Date.now() - 60_000)
    return json({ ok: false, erro: "data_no_passado", detalhe: "a data/hora já passou" });
  ...
  try {
    ...
    if (!corretor)
      return json({
        ...
        detalhe: "telefone não bate com nenhum corretor ativo",
      });
    ...
    const titulo = empreendimento
      ? `Visita - ${empreendimento}`
      : `Visita - ${lead.nome ?? "cliente"}`;
...
async function resolverLead(
  ...
  ref: string,
  corretorId: string,
): Promise<{ lead?: any; erro?: string; detalhe?: string; opcoes?: any[] }> {
  ...
  // por id
  if (UUID.test(ref)) {
    const { data } = await naoLixeira(supabase.from("leads").select(baseSel))
  ...
  if (dig.length >= 8) {
    ...
    if (data && data.length > 1)
      return {
        erro: "lead_ambiguo",
        detalhe: "mais de um lead com esse telefone",
        opcoes: data.map(op),
      };
  }
  ...
  const { data } = await naoLixeira(
    supabase.from("leads").select(baseSel).ilike("nome", `%${termo}%`),
  ).limit(6);
  if (!data || data.length === 0)
    return { erro: "lead_nao_encontrado", detalhe: `nenhum lead do corretor casa com "${ref}"` };
